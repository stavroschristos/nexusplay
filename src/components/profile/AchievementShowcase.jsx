...
// Showcase = hand-picked by user (is_showcased). Falls back to Legendary/Epic.
export default function AchievementShowcase({ achievements, isOwn, onToggleShowcase }) {
  const showcased = achievements?.filter((a) => a.is_showcased) || [];
  const fallback = showcased.length === 0
    ? [...(achievements?.filter((a) => a.rarity === 'Legendary') || []), ...(achievements?.filter((a) => a.rarity === 'Epic') || [])].slice(0, 6)
    : [];
  const display = showcased.length > 0 ? showcased : fallback;
  ...
  if (!display || display.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-xs text-muted-foreground mt-1">{isOwn ? 'Pin your proudest achievements from the Showcase tab in Settings.' : 'This gamer hasn\'t pinned any trophies yet.'}</p>
      </div>
    );
  }
    ...
    <div>
      {showcased.length > 0 && (
        <p className="text-xs text-muted-foreground mb-3 flex items-center gap-1.5">
          <Trophy className="w-3.5 h-3.5 text-amber-400" /> {showcased.length} pinned {showcased.length === 1 ? 'trophy' : 'trophies'}
        </p>
      )}
          ...
          return (
            <div
              ...
              className={cn(
                ...
                config.border, config.grad, config.glow
              )}
            >
              {a.is_showcased && (
                <div className="absolute top-1.5 right-1.5 z-10 px-1.5 py-0.5 rounded-full bg-amber-500/90 text-[8px] font-bold uppercase text-black">Pinned</div>
              )}
              <div className="absolute inset-0 flex flex-col items-center justify-center p-3 text-center">
                <div className="w-12 h-12 rounded-xl bg-black/30 backdrop-blur flex items-center justify-center mb-2">
                  {a.icon_url ? <img src={a.icon_url} alt="" className="w-full h-full rounded-xl object-cover" /> : <Trophy className="w-6 h-6 text-amber-300" />}
                </div>
