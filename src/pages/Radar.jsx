...
import { Loader2, Radio, Gamepad2, Twitch, Users, Filter, Zap, Circle } from 'lucide-react';
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-6 pb-12">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h1 className="text-2xl font-bold font-heading">Gaming Radar</h1>
      </div>
      <p className="text-sm text-muted-foreground mb-6">See who's online and what they're playing right now.</p>

      <div className="flex items-center gap-1.5 mb-6 overflow-x-auto scrollbar-thin pb-1">
        {filters.map((f) => (
          <button key={f.key} onClick={() => setFilter(f.key)} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium whitespace-nowrap transition-all ${filter === f.key ? 'bg-primary text-primary-foreground' : 'bg-secondary/50 text-muted-foreground hover:text-foreground'}`}>
        ...
        <div className="space-y-3">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground text-sm">No one here yet. Follow some gamers to see them on your radar!</p>
                  <Link to="/explore" className="inline-block mt-3 px-4 py-2 rounded-full bg-primary text-primary-foreground text-sm font-medium">Discover Gamers</Link>
                </div>
          ) : filtered.map((p) => (
